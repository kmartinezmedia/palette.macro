...
const convertPalette = (nodes: ObjectProperty[]) => {
  return nodes.reduce((prev, { key, value }) => {
    const name =
      key.type === "Identifier"
        ? (toCssVar(key.name) as CssVariable<PaletteAlias>)
        : "";
    let result;
    if (value.type === "ArrayExpression") {
      const elements = (value.elements as unknown) as [
        { value: PaletteAlias },
        { value: string }
      ];
...
const filterProperties = <
  T extends (ObjectMethod | ObjectProperty | SpreadElement)[]
>(
  props: T
) => {
  return props.filter(
    (item) => item.type === "ObjectProperty"
  ) as ObjectProperty[];
};
